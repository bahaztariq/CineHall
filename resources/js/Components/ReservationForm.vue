...
<template>
    <div class="w-full max-w-4xl bg-zinc-950 border border-zinc-800 p-8 rounded-lg shadow-[0_0_50px_rgba(0,0,0,1)] relative overflow-hidden">
        ...
        <div class="flex justify-between items-end mb-12 border-b border-zinc-900 pb-6">
            ...
            <div class="text-right">
                ...
                <span class="text-2xl font-mono text-cyan-400">{{ (selectedSeats.length * session.price).toFixed(2) }} MAD</span>
            </div>
        </div>

        <div class="w-full h-2 bg-gradient-to-r from-transparent via-cyan-900 to-transparent mb-16 rounded-full shadow-[0_-10px_20px_rgba(34,211,238,0.2)]"></div>

        <div v-if="!loading" class="grid grid-cols-8 gap-3 justify-center mb-12">
            <div 
                v-for="seat in seats" 
                :key="seat.id"
                @click="!seat.is_booked && toggleSeat(seat.id)"
                :class="[
        ...
        <div class="flex gap-4">
            <button @click="$emit('close')" class="flex-1 py-4 bg-zinc-900 text-zinc-500 font-black uppercase italic hover:bg-zinc-800 transition">
                CANCEL ORDER
            </button>
            <button 
